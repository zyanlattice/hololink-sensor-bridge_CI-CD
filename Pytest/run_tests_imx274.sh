#!/bin/bash
# Comprehensive test runner script for Hololink Sensor Bridge verification tests (IMX274 Camera)
...
# Print configuration
echo -e "${BLUE}==================================================================${NC}"
echo -e "${BLUE}Hololink Sensor Bridge Test Suite (IMX274 Camera)${NC}"
echo -e "${BLUE}==================================================================${NC}"
echo ""
...
TEST_LOG_DIR="$TEST_REPORT_DIR/logs_imx274_$TIMESTAMP"
mkdir -p "$TEST_LOG_DIR"
LOG_FILE="$TEST_LOG_DIR/pytest_run.log"

# Setup HTML report in the log directory
if [[ $GENERATE_HTML -eq 1 ]]; then
    HTML_REPORT="$TEST_LOG_DIR/test_report_imx274_$TIMESTAMP.html"
...
if [[ -n "$TEST_FILES" ]]; then
    ...
    # Auto-discover tests: include imx274-specific tests + camera-agnostic tests
    # Pattern: test_*_imx274.py OR test_*.py (without any imx number)
    IMX274_TESTS=""
    for test_file in test_*.py; do
        if [[ -f "$test_file" ]]; then
            # Include if: contains "imx274" OR doesn't contain "imx" followed by digits
            if [[ "$test_file" =~ imx274 ]] || ! [[ "$test_file" =~ imx[0-9]+ ]]; then
                IMX274_TESTS="$IMX274_TESTS $test_file"
            fi
        fi
    done
    
    if [[ -z "$IMX274_TESTS" ]]; then
        echo -e "${RED}Error: No test files found for IMX274${NC}"
        exit 1
    fi
    
    PYTEST_CMD="$PYTEST_CMD $IMX274_TESTS"
    echo -e "${GREEN}Auto-discovered IMX274 tests:${NC} $IMX274_TESTS"
    echo ""
fi
...
cat > "$LOG_FILE" << EOF
==================================================================
Hololink Sensor Bridge - Pytest Execution Log (IMX274 Camera)
==================================================================
...
Test Selection:
  Markers:           ${MARKERS:-all}
  Test Files:        ${TEST_FILES:-IMX274 tests}
...
==================================================================
Hololink Sensor Bridge - Pytest Debug Log (IMX274 Camera)
==================================================================
Test Run Information:
  Date/Time:         $(date '+%Y-%m-%d %H:%M:%S')
  Log Directory:     $TEST_LOG_DIR
...
# Run tests
echo -e "${BLUE}==================================================================${NC}"
echo -e "${BLUE}Starting Tests (IMX274 Camera)${NC}"
echo -e "${BLUE}==================================================================${NC}"
echo ""
