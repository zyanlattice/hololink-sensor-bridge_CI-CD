"""
Test suite for Holoscan Sensor Bridge example applications - IMX274 camera.
...
def run_sample_app(
    ...
    Args:
        script_name: Name of the Python script (e.g., "linux_imx274_player.py")
...
def test_imx274_player(hololink_device_ip, camera_id, record_test_result):
    """
    Test linux_imx274_player.py - Basic IMX274 camera player with Holoviz display.
    
    This test runs the IMX274 player application for a short duration to verify:
    - Camera initialization
    - Frame capture and display
    - Application startup and shutdown
    """
    # Application parameters (linux_imx274_player.py uses different arg names)
    args = [
        "--camera", str(camera_id),  # Uses --camera not --camera-id
        "--camera-mode", "1",  # Mode 1: 1920x1080 @ 60fps (Mode 0 is 4K, heavier)
        "--frame-limit", "200",  # Limit frames to avoid long run time
    ]
    
    # Note: linux_imx274_player.py doesn't accept --camera-ip, uses default 192.168.0.2
    
    # Run the application
    success, stdout, stderr = run_sample_app(
        script_name="linux_imx274_player.py",
        args=args,
        timeout=30
    )
    
    # Check for expected output patterns (linux_imx274_player.py logs to stderr)
    checks = {
...
def test_tao_peoplenet_imx274(hololink_device_ip, camera_id, record_test_result):
    """
    Test linux_tao_peoplenet.py - PeopleNet AI inference for person detection.
    ...
    args = [
        ...
        "--camera-mode", "1",  # Mode 1: 1920x1080 @ 60fps (better for inference)
        "--frame-limit", "500"  # 500 frames for inference testing
    ]
    
    # Note: Script likely uses default IP 192.168.0.2
    
    success, stdout, stderr = run_sample_app(
        script_name="linux_tao_peoplenet.py",
        args=args,
        timeout=120  # Longer timeout for AI model loading and inference
    )
    
    # Check for expected PeopleNet output (IMX274 logs to stderr)
    checks = {
        "app_started": "Initializing" in stderr or "version=" in stderr,
        "graph_executed": "Graph execution finished" in stderr or "Scheduler finished" in stderr,
        "clean_shutdown": "Destroying context" in stderr,
    ...
    all_checks_passed = all(checks.values())
    message = f"TAO PeopleNet IMX274: {'PASS' if success and all_checks_passed else 'FAIL'}"
    if not success:
        message += f" - App returned non-zero exit code"
    failed_checks = [k for k, v in checks.items() if not v]
    if failed_checks:
...
def test_body_pose_imx274(hololink_device_ip, camera_id, record_test_result):
    """
    Test linux_body_pose_estimation.py - Body pose detection with AI inference.
    ...
    """
    args = [
        "--camera-mode", "1",  # Mode 1: 1920x1080 @ 60fps (better for pose estimation)
    ...
    success, stdout, stderr = run_sample_app(
        script_name="linux_body_pose_estimation.py",
        args=args,
        timeout=120  # Longer timeout for AI model loading and inference
    )
    
    # Check for expected body pose output (IMX274 logs to stderr)
    checks = {
        "app_started": "Initializing" in stderr or "version=" in stderr,
        "graph_executed": "Graph execution finished" in stderr or "Scheduler finished" in stderr,
        "clean_shutdown": "Destroying context" in stderr,
    ...
    all_checks_passed = all(checks.values())
    message = f"Body Pose IMX274: {'PASS' if success and all_checks_passed else 'FAIL'}"
    if not success:
        message += f" - App returned non-zero exit code"
    failed_checks = [k for k, v in checks.items() if not v]
    if failed_checks:
...
def test_latency_measurement_imx274(hololink_device_ip, camera_id, record_test_result):
    """
    Test linux_imx274_latency.py - Latency measurement and analysis.
    ...
    # Note: linux_imx274_latency.py uses --hololink for IP (default 192.168.0.2)
    # Camera index is hardcoded to 0 in the script
    
    success, stdout, stderr = run_sample_app(
        script_name="linux_imx274_latency.py",
        args=args,
        timeout=60  # Longer timeout for 200 frames
    )
    
    # Check for expected latency report output (IMX274 logs to stderr)
    checks = {
        "app_started": "Initializing" in stderr or "Calling run" in stderr or "version=" in stderr,
    ...
    all_checks_passed = all(checks.values())
    message = f"Latency Measurement IMX274: {'PASS' if success and all_checks_passed else 'FAIL'}"
    if not success:
        message += f" - App returned non-zero exit code"
    failed_checks = [k for k, v in checks.items() if not v]
    if failed_checks:
