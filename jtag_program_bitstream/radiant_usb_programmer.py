...
# XCF Template for CPNX (CRAM, JTAG)
XCF_TEMPLATE_CPNX_SRAM = """<?xml version='1.0' encoding='utf-8' ?>
<!DOCTYPE        ispXCF    SYSTEM    "IspXCF.dtd" >
<ispXCF version="R2025.2">

    <Comment>Auto-generated by radiant_usb_programmer.py - CPNX Device (SRAM/JTAG)</Comment>
    <Chain>
        <Comm>JTAG</Comm>
        <Device>
...
# XCF Template for CPNX (CRAM, JTAG)
XCF_TEMPLATE_AVANT_CRAM = """<?xml version='1.0' encoding='utf-8' ?>
<!DOCTYPE        ispXCF    SYSTEM    "IspXCF.dtd" >
<ispXCF version="R2025.2">

    <Comment>Auto-generated by radiant_usb_programmer.py - AVANT Device (CRAM/JTAG)</Comment>
    <Chain>
        <Comm>JTAG</Comm>
        <Device>
            <SelectedProg value="TRUE"/>
...
# XCF Template for CPNX (SPI Flash, JTAG2SPI)
XCF_TEMPLATE_CPNX_JTAG2SPI = """<?xml version='1.0' encoding='utf-8' ?>
<!DOCTYPE        ispXCF    SYSTEM    "IspXCF.dtd" >
<ispXCF version="R2025.2">
    <Comment>Auto-generated by radiant_usb_programmer.py - CPNX Device (SPI Flash/JTAG2SPI)</Comment>
    <Chain>
        <Comm>JTAG2SPI</Comm>
        <Device>
            <SelectedProg value="TRUE"/>
            ...
            <File>{bitstream_path}</File>
            <FileTime>{file_time}</FileTime>
            <JedecChecksum>{jedec_checksum}</JedecChecksum>
            <MemoryType>External SPI Flash Memory (SPI FLASH)</MemoryType>
            <Operation>{operation_main}</Operation>
            <Option>
                <SVFVendor>JTAG STANDARD</SVFVendor>
                <AccessMode>Direct Programming</AccessMode>
            </Option>
            <FPGALoader>
                ...
                <FlashDevice>
                    <Device>
                        <Pos>1</Pos>
                        <Vendor>{flash_vendor}</Vendor>
                        <Family>SPI Serial Flash</Family>
                        <Name>{flash_device}</Name>
                        <IDCode>{flash_idcode}</IDCode>
                        <Package>{flash_package}</Package>
                        <Operation>{operation_flash}</Operation>
                        <File>{bitstream_path}</File>
...
XCF_TEMPLATE_AVANT_JTAG2SPI = """<?xml version='1.0' encoding='utf-8' ?>
<!DOCTYPE		ispXCF	SYSTEM	"IspXCF.dtd" >
<ispXCF version="R2025.2">
...
class RadiantUSBProgrammer:
    ...
    def __init__(self, config_path: Optional[str] = None, device_type: Optional[str] = None, communication: Optional[str] = None, verbose: bool = False):
        """
        ...
        Args:
            ...
            communication: 'JTAG' or 'JTAG2SPI' (overrides device default if provided)
        ...
        # Override communication if provided
        if communication:
            self.device_config['communication'] = communication.upper()
    ...
    def generate_xcf(
        ...
        # Select template based on communication field
        comm = self.device_config.get('communication', '').upper()
        if comm == 'JTAG2SPI':
            if self.device_type == 'avant':
                # Use Avant JTAG2SPI template
                template = XCF_TEMPLATE_AVANT_JTAG2SPI
                bitstream_size = bitstream_path.stat().st_size
            else:
                # Use CPNX JTAG2SPI template (external SPI flash)
                template = XCF_TEMPLATE_CPNX_JTAG2SPI
                bitstream_size = bitstream_path.stat().st_size
            template_params = {
                ...
                'jedec_checksum': self.device_config.get('jedec_checksum', '0x21C1'),
                'flash_vendor': self.device_config.get('flash_vendor', 'WinBond'),
                'flash_device': self.device_config.get('flash_device', 'W25Q512JV'),
                'flash_idcode': self.device_config.get('flash_idcode', '0x19'),
                'flash_package': self.device_config.get('flash_package', '8-pad WSON'),
                'flash_address_base': self.device_config.get('flash_address_base', '0x00000000'),
                'flash_address_end': self.device_config.get('flash_address_end', '0x001D0000'),
                'flash_device_size': self.device_config.get('flash_device_size', 513),
                'flash_data_size': bitstream_size
            }
        elif comm == 'JTAG':
            if self.device_type == "avant":
                # Use AVANT CRAM template
                template = XCF_TEMPLATE_AVANT_CRAM
            else:
                # Use CPNX CRAM template
                template = XCF_TEMPLATE_CPNX_SRAM
            template_params = {
                    'device_family': self.device_config['family'],
                    'device_name': self.device_config['name'],
                    'bitstream_path': str(bitstream_path.as_posix()),
...
def main() -> str:
    ...
    parser.add_argument("--communication", type=str,
                        choices=["JTAG", "JTAG2SPI"],
                        help="Communication protocol (JTAG for SRAM, JTAG2SPI for SPI Flash). Overrides device default")
    ...
    if args.fast:
        args.communication = "JTAG"
    else:
        args.communication = "JTAG2SPI"
