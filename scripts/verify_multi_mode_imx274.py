#!/usr/bin/env python3
"""
Multi-mode camera verification script for IMX274.
Runs verify_camera_imx274.py across multiple camera modes sequentially.
"""
...
# IMX274 has 3 modes: 0 (4K 60fps), 1 (1080p 60fps), 2 (4K 60fps 12-bit)
TEST_MODES = [0, 1, 0, 1, 0]

# Find the verify_camera_imx274.py script
def find_camera_script():
    """
    Find verify_camera_imx274.py in the CI_CD/scripts/ folder.
    ...
    for parent in [current_file.parent] + list(current_file.parents):
        ...
        if scripts_dir.exists() and scripts_dir.is_dir():
            camera_script = scripts_dir / "verify_camera_imx274.py"
            if camera_script.exists():
                return str(camera_script)
    
    # Fallback: assume script is in same directory as this file
    fallback = current_file.parent / "verify_camera_imx274.py"
    if fallback.exists():
        return str(fallback)
    
    raise FileNotFoundError("Could not find verify_camera_imx274.py in CI_CD/scripts/ folder")
...
def main() -> tuple[bool, str, dict]:
    ...
    parser = argparse.ArgumentParser(
        description="Run IMX274 camera verification across multiple modes"
    )
    parser.add_argument(
        "--holoviz",
        action="store_true",
