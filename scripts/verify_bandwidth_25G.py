...
def _measure_hololink_throughput(camera_ip: str = "192.168.0.2", frame_limit: int = 300, timeout_seconds: int = 30, camera_mode: int = 4, iface: str = None) -> Tuple[Optional[int], Optional[int]]:
    ...
    try:
        ...
        channel_metadata_0 = hololink_module.Metadata(channel_metadata)
        hololink_module.DataChannel.use_sensor(channel_metadata_0, 0)  # Sensor 0
        
        channel_metadata_1 = hololink_module.Metadata(channel_metadata)
        hololink_module.DataChannel.use_sensor(channel_metadata_1, 1)  # Sensor 1

        # Create SEPARATE data channels
        hololink_channel_0 = hololink_module.DataChannel(channel_metadata_0)
        hololink_channel_1 = hololink_module.DataChannel(channel_metadata_1)

        camera_0 = hololink_module.sensors.imx258.Imx258(hololink_channel_0, camera_id=0)
        camera_1 = hololink_module.sensors.imx258.Imx258(hololink_channel_1, camera_id=0)
        ...
        class ThroughputApplication(holoscan.core.Application):
            ...
            def __init__(self, cuda_ctx, cuda_dev_ord, hl_chan_0, hl_chan_1, cam_0, cam_1, frame_limit):
                super().__init__()
                self._cuda_context = cuda_ctx
                self._cuda_device_ordinal = cuda_dev_ord
                self._hololink_channel_0 = hl_chan_0
                self._hololink_channel_1 = hl_chan_1
                self._camera_0 = cam_0
                self._camera_1 = cam_1
                self._frame_limit = frame_limit
                self._frame_counter_0 = None
                self._frame_counter_1 = None
            ...
            def compose(self):
                ...
                if self._frame_limit:
                    self._count_0 = holoscan.conditions.CountCondition(
                        self,
                        name="count_0",
                        count=self._frame_limit,
                    )
                    condition_0 = self._count_0
                    
                    self._count_1 = holoscan.conditions.CountCondition(
                        self,
                        name="count_1",
                        count=self._frame_limit,
                    )
                    condition_1 = self._count_1
                else:
                    self._ok_0 = holoscan.conditions.BooleanCondition(
                        self, name="ok_0", enable_tick=True
                    )
                    condition_0 = self._ok_0
                    
                    self._ok_1 = holoscan.conditions.BooleanCondition(
                        self, name="ok_1", enable_tick=True
                    )
                    condition_1 = self._ok_1
                ...
                csi_to_bayer_pool = holoscan.resources.BlockMemoryPool(
                    ...
                    name="bayer_pool",
                    storage_type=1,
                    block_size=self._camera_0._width * ctypes.sizeof(ctypes.c_uint16) * self._camera_0._height,
                    num_blocks=6,
                )

                csi_to_bayer_operator_0 = hololink_module.operators.CsiToBayerOp(
                    self,
                    name="csi_to_bayer_0",
                    allocator=csi_to_bayer_pool,
                    cuda_device_ordinal=self._cuda_device_ordinal,
                    out_tensor_name="tensor0",  # Named tensor
                )
                self._camera_0.configure_converter(csi_to_bayer_operator_0)

                csi_to_bayer_operator_1 = hololink_module.operators.CsiToBayerOp(
                    self,
                    name="csi_to_bayer_1",
                    allocator=csi_to_bayer_pool,
                    cuda_device_ordinal=self._cuda_device_ordinal,
                    out_tensor_name="tensor1",  # Named tensor
                )
                self._camera_1.configure_converter(csi_to_bayer_operator_1)


                frame_size = csi_to_bayer_operator_0.get_csi_length()
                assert frame_size == csi_to_bayer_operator_1.get_csi_length()
                ...
                receiver_operator_0 = hololink_module.operators.LinuxReceiverOperator(
                    self,
                    condition_0,
                    name="receiver_0",
                    frame_size=frame_size,
                    frame_context=self._cuda_context,
                    hololink_channel=self._hololink_channel_0,
                    device=self._camera_0
                )

                receiver_operator_1 = hololink_module.operators.LinuxReceiverOperator(
                    self,
                    condition_1,
                    name="receiver_1",
                    frame_size=frame_size,
                    frame_context=self._cuda_context,
                    hololink_channel=self._hololink_channel_1,
                    device=self._camera_1,
                )

                # Frame counter as terminal operator (no CsiToBayerOp in flow for stable throughput)
                self._frame_counter_0 = FrameCounterOp(
                    self,
                    name="frame_counter_0",
                    frame_limit=self._frame_limit,
                    pass_through=False
                )

                self._frame_counter_1 = FrameCounterOp(
                    self,
                    name="frame_counter_1",
                    frame_limit=self._frame_limit,
                    pass_through=False
                )
                
                self.add_flow(receiver_operator_0, self._frame_counter_0, {("output", "input")})
                self.add_flow(receiver_operator_1, self._frame_counter_1, {("output", "input")})
            
            def get_frame_count(self) -> int:
                return (self._frame_counter_0.frame_count if self._frame_counter_0 else 0) + (self._frame_counter_1.frame_count if self._frame_counter_1 else 0)
            
            def get_fps(self) -> float:
                if (not self._frame_counter_0 or not self._frame_counter_0.start_time or not self._frame_counter_0.end_time) and (not self._frame_counter_1 or not self._frame_counter_1.start_time or not self._frame_counter_1.end_time):
                    return 0.0
                elapsed = self._frame_counter_0.end_time - self._frame_counter_0.start_time
                # Fence post: N frames span N-1 intervals
                return ((self._frame_counter_0.frame_count) / elapsed if elapsed > 0 and self._frame_counter_0.frame_count > 1 else 0.0) + ((self._frame_counter_1.frame_count) / elapsed if elapsed > 0 and self._frame_counter_1.frame_count > 1 else 0.0)
        ...
        hololink = hololink_channel_0.hololink()
        assert hololink is hololink_channel_1.hololink()  # Verify they're the same
        ...
        camera_0.set_exposure(0x0600)
        camera_0.set_analog_gain(0x0180)

        print("Data Generator Sensor 0")
        ...
        hololink.write_uint32(0x01000110,0x3333)#Set Data Gen SIF CLK Divider, SIF_CLK/((65536/0xC000)) Orin - 0xA8F6, Thor - 0xE666
        hololink.write_uint32(0x01000104,0x03)#Enable Data Gen with Continuous Mode	    

        print("Data Generator Sensor 1")
        hololink.write_uint32(0x01010104,0x00)#Disable Data Gen 
        hololink.write_uint32(0x01010108,0x01)#Set Data Gen to Counter Mode , 0: PRBS, 1: Counter
        hololink.write_uint32(0x01010110,0x3333)#Set Data Gen SIF CLK Divider, SIF_CLK/((65536/0xC000)) Orin - 0xA8F6, Thor - 0xE666
        hololink.write_uint32(0x01010104,0x00)#Enable Data Gen with Continuous Mode	
        ...
        fc = application._frame_counter_0  # Could also check _frame_counter_1, but they should be in sync since same CountCondition
