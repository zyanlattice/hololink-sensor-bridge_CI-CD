...
def _measure_hololink_throughput(camera_ip: str = "192.168.0.2", frame_limit: int = 300, timeout_seconds: int = 15, camera_mode: int = 1 ) -> Tuple[Optional[int], Optional[int]]:
    """
    Measure actual hololink throughput by receiving frames from IMX274 camera.
    ...
    Args:
        ...
        camera_mode: IMX274 camera mode (default 1 = 1920x1080 60fps)
    ...
    try:
        ...
        # Start Hololink and camera (exactly like verify_camera_imx274.py)
        logging.info("Starting Hololink and camera...")
        hololink = hololink_channel.hololink()
        hololink.start()
        hololink.reset()  # IMX274 requires reset after start
        
        # IMX274 camera setup sequence
        camera.setup_clock()
        camera.configure(camera_mode_enum)
        camera.set_digital_gain_reg(0x4)
        
        # Note: IMX274 does NOT have motorized focus like IMX258, so no set_focus() call
        
        # Brightness adjustment
        camera.set_exposure_reg(0x0600)
        camera.set_analog_gain_reg(0x0180)
        
        # Note: IMX274 does NOT call camera.start() - it starts when application.run() is called
        ...
        if elapsed_time > 0 and frame_count > 0:
            ...
            logging.info(f"  Frame size: {frame_size} bits")
...
def argument_parser() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify IMX274 camera Ethernet speed functionality")
    parser.add_argument("--camera-ip", type=str, default="192.168.0.2", help="Hololink device IP")
    parser.add_argument("--camera-mode", type=int, default=1, help="Camera mode (default 1 = 1920x1080 60fps)")
