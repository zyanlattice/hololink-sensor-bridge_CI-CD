...
class FrameCounterOp(holoscan.core.Operator):
    ...
    def compute(self, op_input, op_output, context):
        ...
        try:
            ...
            # Validate timestamps are reasonable (within 10 years of epoch 2020-2040, roughly 1.5-2.5 billion seconds)
            # Garbage timestamps will be billions of seconds (e.g., 5910139803 = year 2157)
            current_time_s = time.time()
            min_valid_time = 1577836800  # 2020-01-01 00:00:00 UTC
            max_valid_time = 2208988800  # 2040-01-01 00:00:00 UTC
            
            def is_valid_timestamp(ts):
                """Check if timestamp is within reasonable range (2020-2040)."""
                return ts is not None and min_valid_time <= ts <= max_valid_time
            
            # Validate all timestamps - if any are invalid, mark entire frame as invalid
            if not all(is_valid_timestamp(ts) for ts in [frame_start_s, frame_end_s, received_s, operator_s, complete_s]):
                if self.frame_count == 1:
                    logging.warning(f"⚠️  Invalid PTP timestamps detected (frame_end={frame_end_s:.3f}s, current={current_time_s:.3f}s)")
                    logging.warning(f"    IMX274 receiver may not populate PTP timestamps correctly")
                # Append None for invalid data
                ...
                # All timestamps valid, append them
                ...
                if self.frame_count == 1:
                    logging.info(f"✓ PTP metadata available (all 5 timestamps recorded and valid)")
    ...
    def get_ptp_timing_stats(self) -> dict:
        ...
        # Calculate all 5 latency metrics for each frame (like linux_imx274_latency.py)
        ...
        for i in valid_indices:
            ...
            frame_time_dt = frame_end - frame_start                 # Sensor readout + FPGA processing
        ...
        # Mode-aware tolerance: all IMX274 modes are 60fps = 16.67ms
...
def _measure_hololink_ptp(camera_ip: str = "192.168.0.2", frame_limit: int = 300, timeout_seconds: int = 15, camera_mode: int = 1 ) -> Tuple[Optional[int], Optional[int]]:
    """
    Measure actual hololink PTP latency by receiving frames from IMX274 camera.
    Uses exact same initialization as verify_camera_imx274.py, just counts frames without processing.
    
    Args:
        camera_ip: IP of Hololink device
        frame_limit: Number of frames to receive
        timeout_seconds: Maximum time to wait
        camera_mode: IMX274 camera mode (0=4K 60fps, 1=1080p 60fps, 2=4K 60fps 12-bit)
        
    Returns:
        PTP timing statistics dict, or None if measurement failed
    """
    ...
    try:
        logging.info(f"Measuring hololink PTP latency: {frame_limit} frames from {camera_ip}...")
        
        # Initialize CUDA (exactly like verify_camera_imx274.py)
        ...
        # Find Hololink channel (exactly like verify_camera_imx274.py)
        ...
        # Initialize IMX274 camera (exactly like verify_camera_imx274.py)
        ...
        class ThroughputApplication(holoscan.core.Application):
            """Minimal app for PTP latency measurement."""
            
            def __init__(self, cuda_ctx, cuda_dev_ord, hl_chan, cam, cam_mode, frame_limit):
                super().__init__()
            ...
            def compose(self):
                # IMX274 requires set_mode() in compose
                # Convert camera_mode int to enum (required by IMX274 driver)
                camera_mode_enum = hololink_module.sensors.imx274.imx274_mode.Imx274_Mode(self._camera_mode)
                self._camera.set_mode(camera_mode_enum)
                
                # Use CountCondition to limit frames (like linux_imx274_player.py)
        ...
        # Start Hololink and camera (MUST follow IMX274 sequence)
        logging.info("Starting Hololink...")
        hololink = hololink_channel.hololink()
        hololink.start()
        
        # IMX274 requires reset after hololink.start()
        hololink.reset()

        # CRITICAL: Synchronize PTP clock before reading timestamps
        logging.info("Waiting for PTP synchronization...")
        if not hololink.ptp_synchronize():
            logging.error("Failed to synchronize PTP clock")
            return None
        ...
        # Set exposure and gain (IMX274 uses _reg methods)
        camera.set_exposure_reg(0x0600)
        camera.set_analog_gain_reg(0x0180)
        # NO camera.start() for IMX274 - starts automatically
        ...
        if ptp_stats and "error" not in ptp_stats:
            ...
            logging.info(f"   Mean: {ptp_stats['mean_frame_acquisition_ms']:.3f} ms (sensor readout + FPGA ~4-5ms)")
...
def argument_parser() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify IMX274 camera PTP timing")
    parser.add_argument("--camera-ip", type=str, default="192.168.0.2", help="Hololink device IP")
    parser.add_argument("--camera-mode", type=int, default=0, help="Camera mode (0=4K 60fps, 1=1080p 60fps, 2=4K 60fps 12-bit)")
...
def main() -> Tuple[bool, str, dict]:
    """
    Verify PTP timing for IMX274 camera.
    ...
    # Measure actual PTP latency (requires running hardware)
    ...
    if result is None:
        ...
        # PTP timestamps not available or insufficient data
        ptp_pass = False
        stats = {
            "camera_ip": cam_ip,
            "camera_mode": cam_mode,
            "frame_limit": frame_lim,
            "frames_captured": result.get('frame_count', 0),
        ...
        print(f"📊 Metrics: {stats}")
        return ptp_pass, f"PTP timing unavailable: {result['error']}", stats
    else:
        ...
        # Pass criteria: Check inter-frame interval (not frame acquisition time)
        # IMX274 all modes are 60fps = 16.67ms interval
        expected_interval_ms = 16.67
        tolerance = 0.1  # 10% tolerance
        ptp_pass = (mean_frame_interval >= expected_interval_ms * (1 - tolerance)) and \
                   (mean_frame_interval <= expected_interval_ms * (1 + tolerance))  
        ...
        if ptp_pass:
            return ptp_pass, f"Hololink PTP measurement passed (Frame interval={mean_frame_interval:.2f}ms, Frame acquisition={mean_frame_acquisition:.2f}ms, Expected interval: 16.67±10%ms)", stats
        else:
            return False, f"Hololink PTP measurement failed: Frame interval={mean_frame_interval:.3f}ms (Expected: 16.67±10%ms)", stats
