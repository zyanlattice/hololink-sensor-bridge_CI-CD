...
class FrameCounterOp(holoscan.core.Operator):
    ...
    def get_ptp_timing_stats(self, drop_frames=5) -> dict:
        ...
        for i in valid_indices:
            ...
            frame_time_dt = frame_end - frame_start  # Expected: ~19.4ms for IMX258
...
def main() -> Tuple[bool, str, dict]:
    ...
    if result is None:
        ...
        # 1. Frame acquisition time within ±10% of expected (19.4ms for IMX258)
        # 2. Network latency < 12ms (Linux UDP threshold)
        # 3. Inter-frame jitter acceptable (< 10% of frames outside tolerance)
        ptp_pass = (
            (15.8 * 0.9 <= mean_frame_acquisition <= 15.8 * 1.1) and  # Frame acq time OK
        ...
        if ptp_pass:
            ...
            failure_reasons = []
            if not (15.8 * 0.9 <= mean_frame_acquisition <= 15.8 * 1.1):
                failure_reasons.append(f"Frame acquisition={mean_frame_acquisition:.2f}ms (expected 15.8±10%)")
